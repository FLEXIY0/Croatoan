...
### Меню, скриншот и биндинги

Файлы Openbox (`menu.xml`, `rc.xml`) принадлежат пакету `cbpp-configs`.
Переопределять их целиком из этого репозитория означало бы забрать себе всё
меню CBPP и все его будущие изменения — вместо этого два хука **правят**
установленные файлы.

Это работает потому, что `chroot_hooks` в live-build выполняется **после**
`chroot_includes_after_packages` и после установки пакетов, то есть хук видит
файлы, которые положил `cbpp-configs`.

- `0995-openbox-screenshot` — добавляет пункт `Screenshot` в корень меню,
  сразу за «Media Player». Запускает `/usr/bin/cbpp-screenshot`: снимок всего
  экрана в `~/images`, уведомление, и никакого просмотрщика поверх только что
  снятого экрана.
- `0996-openbox-keybinds` — добавляет `W-q`, `W-Return`, `W-1`/`W-2`,
  `W-S-1`/`W-S-2`, `W-S-q`. Все эти клавиши в поставляемом `rc.xml` свободны,
  так что ничего существующего не переназначается. Единственное исключение —
  `Print`: он открывал диалог `xfce4-screenshooter`, теперь просто делает
  снимок. Диалог остался в меню.

Оба хука идемпотентны, оба падают с внятной ошибкой, если якорь в файле не
найден — лучше уронить сборку, чем молча потерять функцию, — и оба проверяют
результат через `xmllint`, если он есть. Ради этого в список пакетов добавлен
`libxml2-utils`: сломанный `rc.xml` ломает рабочий стол целиком, и узнать об
этом на сборке несравнимо лучше, чем в готовом образе.

Проверено на настоящих файлах из `cbpp-configs_13.0_all.deb`: оба хука
отрабатывают, `menu.xml` и `rc.xml` остаются well-formed, повторный запуск
ничего не дублирует. Схемы `menu.xsd`/`rc.xsd` Debian в пакете openbox не
поставляет, поэтому дальше well-formed проверить нечем.
