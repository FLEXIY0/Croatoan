...
Исходник лежит **без цвета** в `assets/logo-plain.txt`, а
`scripts/cbpp-logo-color` красит его и пишет ANSI-файл, который едет в образ.
Схема выбирается одной строкой `CBPP_LOGO_STYLE` в `cbpp.conf`; доступны
`skull-tentacles`, `outline`, `relief`, `glow`, `gradient`, `blood`, `mono`.
Посмотреть все сразу:

```sh
scripts/cbpp-logo-color --preview assets/logo-plain.txt /tmp/p.html
```

Символы `0` и `1` в арте — это дизеринг, а не полутона: у клеток обоих типов
одинаковое среднее число соседей (5.5 против 5.9). Поэтому раскраска идёт по
геометрии — по толщине массы (так отделяется череп от щупалец) и по глубине
залегания клетки под силуэтом (так получаются кромка и тени).

Используются только 16 базовых ANSI-цветов: баннер печатается и в виртуальной
консоли, а она 256 цветов не умеет.

Тип лого в конфиге fastfetch — `file-raw`, а не `file`: `file` заменил бы наши
escape-последовательности одним плоским цветом. Ширину fastfetch при этом
считает по видимым символам, поэтому блок информации остаётся выровненным.

### Обои

`CBPP_WALLPAPER` в `cbpp.conf`. Картинка кладётся в
`includes.chroot_after_packages/usr/share/backgrounds/` — туда же, куда ставит
свои `cbpp-wallpapers`, и именно этот каталог просматривает nitrogen.
Генерируется `/etc/skel/.config/nitrogen/bg-saved.cfg`, который восстанавливает
`nitrogen --restore` из openbox-автостарта.

Режим пишется голым числом, и порядок зафиксирован в исходниках nitrogen
(`src/SetBG.h`, с комментарием «do not change the order»): `0` scale, `1` tile,
`2` center, `3` zoom, `4` **auto**, `5` zoom-fill. Обрати внимание: `4` — это
не zoom-fill, как можно подумать. По умолчанию стоит `3` — сохраняет пропорции
и не обрезает надпись, а чёрные поля не видны на чёрном фоне.
